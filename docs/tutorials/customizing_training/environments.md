...
* `Node(context: str, audit: str, response: str, reward: float, children: list[Node])`
...
Each Node records the conversation state so far in context, the auditor's next audit, the defender's response, a per-turn scalar reward from your System, and its own children (the next layer of nodes). With tree_depth = 3, the rollout contains 3 auditor–defender turns along any path from the root, branching 2 ways at each auditor step; leaf nodes are those with children=[].

In short, it's a depth-3, width-2 conversation tree rooted at the initial prompt, where each node captures the audit, response, reward, and the updated context that feeds the next expansion.
```
</details>
...
* `audit` — auditor's utterance / action,
...
* `reward(prompts, audits, responses) -> Sequence[float]`
* `advance(prompt, audit, response) -> str` *(builds the next state)*
...
* **Check your sampler by printing a few rollouts.**
  ...
  for i, n in enumerate(g.children):
      print(f"[{i}] audit={n.audit!r} rsp={n.response!r} "
...
class CustomNode(Node[str, str]):
    ...
    def __init__(
        ...
        *,
        ...
        audit_logprob: float = 0.0,
    ):
        # Initialize the base Node fields first
        super().__init__(context, audit, response, reward, list(children))
        # Then attach your custom metrics
        self.audit_tox: float = float(audit_tox)
        self.target_tox: float = float(target_tox)
        self.audit_logprob: float = float(audit_logprob)
```

...
* `Node(context: str, challenge: str, response: str, reward: float, children: list[Node])`
...
Each Node records the conversation state so far in context, the tester's next challenge, the defender's response, a per-turn scalar reward from your System, and its own children (the next layer of nodes). With tree_depth = 3, the rollout contains 3 tester–defender turns along any path from the root, branching 2 ways at each tester step; leaf nodes are those with children=[].

In short, it's a depth-3, width-2 conversation tree rooted at the initial prompt, where each node captures the challenge, response, reward, and the updated context that feeds the next expansion.
```
</details>
...
* `challenge` — auditor's challenge / action,
* `response` — target/defender's response / reply,
...
* `reward(prompts, challenges, responses) -> Sequence[float]`
* `advance(prompt, challenge, response) -> str` *(builds the next state)*
...
* **Check your sampler by printing a few rollouts.**
  ...
  for i, n in enumerate(g.children):
      print(f"[{i}] challenge={n.challenge!r} rsp={n.response!r} "
...
class CustomNode(Node[str, str]):
    ...
    def __init__(
        ...
        *,
        challenge_tox: float = 0.0,
        target_tox: float = 0.0,
        challenge_logprob: float = 0.0,
    ):
        # Initialize the base Node fields first
        super().__init__(context, challenge, response, reward, list(children))
        # Then attach your custom metrics
        self.challenge_tox: float = float(challenge_tox)
        self.target_tox: float = float(target_tox)
        self.challenge_logprob: float = float(challenge_logprob)
```
